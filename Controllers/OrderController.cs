...
namespace StoreFlow.Controllers
{
    public class OrderController : Controller
    {
        ...
        {
            var values = _context.Orders.Where(x => x.Status.Contains(status)).Include(x => x.Customer).Include(x => x.Product).ToList();
            if (!values.Any())
            {
                ViewBag.v = "Bu Status İle İlgili Veri Bulunamadı!";
            }
            return View(values);
        }
